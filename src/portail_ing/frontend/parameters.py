...
class Analysis(BaseParameterized):
    ...
    def perform_checks(self, uuid, kind, da, updaters: dict = {}):
        ...
        if (kind == "obs") and (self.checked[uuid][kind]["ref_period"] != ref_period):
            ...
            self.checked[uuid][kind]["ref_period"] = updaters["ref_period"]
...
class IndicatorDA(BaseParameterized):
    ...
    analysis = param.ClassSelector(class_=Analysis, doc="Analysis parent")
    ...
    @property
    def period(self):
        return self.analysis.ref_period
    
    @period.setter
    def period(self,period):
        self.analysis.ref_period = period
...
class IndicatorObsDA(IndicatorDA):
    ...
    @param.depends("dist", "analysis.ref_period", watch=True, on_init=True)
    ...
    @param.depends("analysis.ref_period", "metric", watch=True)
...
class IndicatorSimDA(IndicatorDA):
    ...
    @property
    def period(self):
        return self.analysis.fut_period

    @period.setter
    def period(self,period):
        self.analysis.fut_period = period
    ...
    @param.depends("dist", "analysis.fut_period", "ts", watch=True, on_init=True)
    def _update_params(self):
        """Update the distribution parameters."""
    ...
    @param.depends("analysis.fut_period", watch=True, on_init=True)
...
class IndicatorRefDA(IndicatorSimDA):
    ...
    @param.depends("dist", "analysis.ref_period", "ts", watch=True, on_init=True)
    def _update_params(self):
        """Update the distribution parameters."""
        if self.dist is not None and self.ts is not None:
            with param.parameterized.discard_events(self):
    ...
    @param.depends("analysis.ref_period", "level", watch=True, on_init=True)
    def _update_ks(self):
    ...
    def _update_model_weights(self):
        ...
        if len(ok) < 2:
            new_period = (max(self.period[0] - 1,self.analysis.param.ref_period.bounds[0]), min(self.period[1] + 1, self.analysis.param.ref_period.bounds[1]))
            logger.warning(f"Not enough models to compute weights, attempting to extend ref_period from {self.period} to {new_period}")
            if self.model_weights is not None:
                with param.parameterized.discard_events(self):
